...
// Función para imprimir headers de seguridad
function printSecurityHeaders(headers) {
  ...
  console.log(`   ${colors.yellow}Headers de Seguridad:${colors.reset}`);
  securityHeaders.forEach(header => {
    if (headers[header]) {
      console.log(`   ${colors.green}✓${colors.reset} ${header}: ${headers[header]}`);
    } else {
      console.log(`   ${colors.red}✗${colors.reset} ${header}: No presente`);
    }
  });
}
...
async function testPublicEndpoints() {
  console.log(`\n${colors.bright}${colors.blue}=== TESTING ENDPOINTS PÚBLICOS ===${colors.reset}\n`);

  // Test 1: Login endpoint - método GET (debería redirigir o devolver 405)
  try {
    const response = await makeRequest(`${BASE_URL}/api/auth/login`, { method: 'GET' });
    const passed = response.status === 405 || response.status === 404;
    printResult('Login GET Method', passed, `Status: ${response.status}`);
  } catch (error) {
    printResult('Login GET Method', false, `Error: ${error.message}`);
  }

  // Test 2: Login endpoint - POST sin datos
  try {
    ...
    printSecurityHeaders(response.headers);
  } catch (error) {
    printResult('Login POST sin datos', false, `Error: ${error.message}`);
  }

  // Test 3: Login endpoint - POST con datos inválidos
  try {
    const response = await makeRequest(`${BASE_URL}/api/auth/login`, { 
      method: 'POST',
      body: {
  ...
  // Test 4: Register endpoint - POST sin datos
  try {
  ...
  // Test 5: Register endpoint - POST con datos inválidos
  try {
    const response = await makeRequest(`${BASE_URL}/api/auth/register`, { 
      method: 'POST',
      body: {
    ...
    const passed = response.status === 400;
    printResult('Register POST datos inválidos', passed, `Status: ${response.status}`);
  } catch (error) {
    printResult('Register POST datos inválidos', false, `Error: ${error.message}`);
  }

  // Test 6: Logout endpoint - POST sin autenticación
  try {
...
async function testProtectedEndpointsWithoutAuth() {
  console.log(`\n${colors.bright}${colors.blue}=== TESTING ENDPOINTS PROTEGIDOS SIN AUTENTICACIÓN ===${colors.reset}\n`);

  const protectedEndpoints = [
    '/api/me',
    '/api/employees',
    '/api/employees/123e4567-e89b-12d3-a456-426614174000',
    '/api/dashboard/stats',
    '/api/time-entries',
    '/api/roles',
    '/api/holidays',
...
async function testMaliciousInjection() {
  console.log(`\n${colors.bright}${colors.blue}=== TESTING INYECCIÓN DE DATOS MALICIOSOS ===${colors.reset}\n`);
  ...
  const maliciousPayloads = [
    ...
    { email: 'test@test.com; rm -rf /', password: 'password123' },
    { email: 'test@test.com', password: 'password123; rm -rf /' },
    
    // Path Traversal attempts
    { email: '../../../etc/passwd', password: 'password123' },
    { email: 'test@test.com', password: '../../../etc/passwd' }
  ];
  ...
  for (let i = 0; i < maliciousPayloads.length; i++) {
    ...
    try {
      ...
      printResult(`Inyección maliciosa ${i + 1}`, passed, `Status: ${response.status} - Email: ${payload.email.substring(0, 20)}...`);
    } catch (error) {
...
async function testRateLimiting() {
  console.log(`\n${colors.bright}${colors.blue}=== TESTING RATE LIMITING ===${colors.reset}\n`);

  // Test rate limiting en login
  console.log('Probando rate limiting en login endpoint...');
  ...
  for (let i = 0; i < 15; i++) {
    try {
      ...
      if (response.status === 429) {
        rateLimitHit = true;
        printResult('Rate limiting activo', true, `Rate limit alcanzado en intento ${i + 1}`);
        break;
      }
    } catch (error) {
      // Ignorar errores de conexión durante rate limiting
    }
  }
  
  if (!rateLimitHit) {
    printResult('Rate limiting', false, 'Rate limiting no detectado después de 15 intentos');
  }
}

// Tests de headers de seguridad
async function testSecurityHeaders() {
  console.log(`\n${colors.bright}${colors.blue}=== TESTING HEADERS DE SEGURIDAD ===${colors.reset}\n`);
  ...
  try {
    ...
    requiredHeaders.forEach(header => {
      if (!response.headers[header]) {
        ...
        printResult(`Header ${header}`, false, 'No presente');
      } else {
        printResult(`Header ${header}`, true, response.headers[header]);
      }
    });

    printResult('Todos los headers de seguridad', allHeadersPresent, 
      allHeadersPresent ? 'Todos los headers requeridos están presentes' : 'Faltan algunos headers de seguridad');

  } catch (error) {
    printResult('Headers de seguridad', false, `Error: ${error.message}`);
  }
}
...
// Tests de tamaño de request
async function testRequestSizeValidation() {
  console.log(`\n${colors.bright}${colors.blue}=== TESTING VALIDACIÓN DE TAMAÑO DE REQUEST ===${colors.reset}\n`);

  // Test con request muy grande
  const largePayload = {
    email: 'a'.repeat(10000),
    password: 'b'.repeat(10000),
    companyName: 'c'.repeat(10000),
    firstName: 'd'.repeat(10000),
    lastName: 'e'.repeat(10000)
  };
  ...
  try {
    ...
    printResult('Request muy grande', passed, `Status: ${response.status}`);
  } catch (error) {
    printResult('Request muy grande', true, 'Request rechazado por tamaño');
  }

  // Test con request con caracteres especiales
  const specialCharsPayload = {
    email: 'test@test.com',
    password: 'password123',
    companyName: 'Company with special chars: !@#$%^&*()_+-=[]{}|;:,.<>?',
    firstName: 'Name with special chars: !@#$%^&*()_+-=[]{}|;:,.<>?',
    lastName: 'Last with special chars: !@#$%^&*()_+-=[]{}|;:,.<>?'
  };
  ...
  try {
    ...
    const passed = response.status === 400;
    printResult('Request con caracteres especiales', passed, `Status: ${response.status}`);
  } catch (error) {
    printResult('Request con caracteres especiales', false, `Error: ${error.message}`);
  }
}

// Función principal
async function runAllTests() {
  console.log(`${colors.bright}${colors.magenta}🚀 INICIANDO PRUEBAS DE SEGURIDAD DE ENDPOINTS${colors.reset}`);
  ...
  try {
    ...
    await testRequestSizeValidation();
    ...
    console.log(`${colors.yellow}Revisa los resultados arriba para verificar la seguridad de los endpoints.${colors.reset}`);
